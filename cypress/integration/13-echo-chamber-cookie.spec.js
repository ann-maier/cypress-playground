...
describe('Setting the cookie', () => {
  beforeEach(() => {
    cy.task('seed');
    cy.setCookie('jwt', encodeToken({ id: 1, email: 'test@example.com' }));
    cy.visit('/echo-chamber/sign-in');
  });
...
describe('Setting the cookie with real data', () => {
  beforeEach(() => {
    cy.task('seed');
    cy.request('/echo-chamber/api/users').then((response) => {
      const [user] = response.body.users;
      cy.setCookie('jwt', encodeToken(user)).then(() => user).as('user');
    });
    cy.visit('/echo-chamber/sign-in');
  });
